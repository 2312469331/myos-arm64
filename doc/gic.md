...
下面是这段内容的完整翻译，我尽量保持了技术文档的严谨性和术语的准确性，同时补充了少量必要的说明：

---

### **GICD_ITARGETSR 寄存器特性**

#### **用途**
GICD_ITARGETSR 为 GIC 所支持的每个中断提供一个 **8 位 CPU 目标字段**。该字段存储中断的**目标处理器列表**，即当中断被触发且优先级足够时，分发器（Distributor）会将该中断转发到哪些 CPU 接口。

#### **使用约束**
在**多核实现**中：
- 这些寄存器支持**字节访问**。
- 对应未实现中断的寄存器字段为 **RAZ/WI**（读恒为 0，写忽略）。
- `GICD_ITARGETSR0` 至 `GICD_ITARGETSR7` 为只读寄存器，其每个字段返回的值仅对应于正在读取该寄存器的处理器。
- 哪些（如果有的话）SPI（共享外设中断）在硬件中被静态配置，这由**实现定义**。此类 SPI 的 CPU 目标字段为只读，返回值表示该中断的 CPU 目标。
- 如果 GIC 实现了安全扩展：
  - 对应 Group 0 中断的寄存器字段，对非安全访问为 **RAZ/WI**。
  - 如果 GIC 实现了配置锁定功能，系统可以为配置为 Group 0 的可锁定 SPI 锁定其 CPU 目标字段，详见第 4-82 页的“配置锁定”一节。

另见第 4-108 页的《对 GICD_ITARGETSR 进行更改的影响》一节。

**注意**
在单核实现中，所有中断都以唯一的处理器为目标，因此 GICD_ITARGETSR 寄存器均为 **RAZ/WI**。

#### **配置情况**
这些寄存器在 GIC 的所有配置中均可用。如果 GIC 实现了安全扩展，这些寄存器属于**通用（Common）**寄存器（即安全和非安全模式下均可访问）。

已实现的 GICD_ITARGETSR 寄存器数量为：
`8 * (GICD_TYPER.ITLinesNumber + 1)`
寄存器编号从 `GICD_ITARGETSR0` 开始向上递增。

在多核实现中，`GICD_ITARGETSR0` 至 `GICD_ITARGETSR7` 为每个连接的处理器提供**分银行（banked）寄存器**。这些寄存器保存中断 0–31 的 CPU 目标字段。

#### **属性**
详见第 4-75 页的表 4-1 寄存器摘要。

图 4-14 展示了多核实现中 GICD_ITARGETSR 的位分配。对应未实现 CPU 接口的 CPU 目标字段位为 **RAZ/WI**。

对于中断 ID 为 `m` 的中断，使用整数除法（DIV）和取模（MOD）运算：
- 对应的 `GICD_ITARGETSRn` 寄存器编号 `n` 为：`n = m DIV 4`
- 所需 `GICD_ITARGETSR` 寄存器的地址偏移为：`0x800 + (4 * n)`
- 所需 CPU 目标字段在该寄存器中的字节偏移为：`m MOD 4`，其中：
  - 字节偏移 0 对应寄存器位 `[7:0]`
  - 字节偏移 1 对应寄存器位 `[15:8]`
  - 字节偏移 2 对应寄存器位 `[23:16]`
  - 字节偏移 3 对应寄存器位 `[31:24]`
...
### **对 GICD_ITARGETSR 进行更改的影响**
软件可随时对 GICD_ITARGETSR 寄存器执行写操作。CPU 目标字段值的任何更改：
- 对**已激活（active）的中断无影响**。这意味着，从目标列表中移除一个 CPU 接口，不会取消该中断在该 CPU 接口上的激活状态。
- 对**待处理（pending）的中断有影响**。这意味着：
  - 向待处理中断的目标列表中添加一个 CPU 接口，会使该中断在该 CPU 接口上变为待处理状态。
  - 从待处理中断的目标列表中移除一个 CPU 接口，会清除该中断在该 CPU 接口上的待处理状态。

**注意**
任何更改生效都需要有限的时间。
- 如果更改应用于一个同时处于激活和待处理状态的中断，则直到激活状态被清除后，中断的目标才会发生改变。

---

### 补充术语说明
- **RAZ/WI**：Read As Zero / Write Ignored，即读操作恒返回 0，写操作被硬件忽略。
- **SPI**：Shared Peripheral Interrupt，共享外设中断，即多个处理器共享的外设中断。
- **Group 0/1**：安全扩展中定义的中断组，Group 0 为安全中断，Group 1 为非安全中断。
- **Banked registers**：分银行寄存器，指每个核心都有自己独立的副本，核心只能访问自己的那一份。

需要我帮你把这段内容整理成一份**更简洁的要点笔记**，方便你直接用在学习笔记里吗？
