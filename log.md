...
# 提取完整文字
根本原因：
1. Clang 把局部数组 `uint32_t timer_irqs[] = {26, 27, 28, 29}` 优化成了静态常量，放进了 `.kernel` 段（地址 `0xFFFF_8000_402E_6C80`，这是虚拟地址）
2. `gic_el3_init` 在 EL3 阶段被调用，此时 MMU 还没开，CPU 直接使用物理地址
3. 物理地址空间只有 `0x40000000` 开始的 256MB，而 `0xFFFF_8000_402E_6C80` 远超物理内存范围 →Data Abort →崩溃

GCC 会老老实实把 `timer_irqs` 放在栈上（用 sp 偏移访问），Clang 却"自作聪明"把它提升为 `.kernel` 段的静态常量，用绝对虚拟地址加载。

修复方案：直接内联常量，不给 Clang 优化机会：
