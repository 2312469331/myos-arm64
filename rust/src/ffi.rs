...
unsafe extern "C" {
    ...
    pub fn dma_alloc_coherent(size: u64, dma_phys: *mut u64) -> *mut u8;
    pub fn dma_free_coherent(va: *mut u8);
    ...
    pub fn map_page(va: usize, pa: u64, prot: u64) -> i32;
    /// 取消一个 4KB 页面映射
    pub fn unmap_page(va: usize);
    /// 从页表中读取虚拟地址对应的物理地址，未映射返回 0
    pub fn get_phys_from_va(va: usize) -> u64;
    /// 复制用户态页表树（共享物理页，用于 fork）
    pub fn copy_user_pgd(src_pgd_pa: u64, dst_pgd_pa: u64);
    /// 递归释放整个页表树（只释放页表页，不释放映射的数据页）
    pub fn free_pgd_tree(pgd_pa: u64);
    /// 调试：打印页表树状结构
