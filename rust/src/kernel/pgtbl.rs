...
/// DMA 缓冲（Normal Non-cacheable，AttrIndx=1）
pub const PAGE_DMA: u64 =
    (1 << 0)   // Valid
    | (1 << 1)  // Page
    | (1 << 10) // AF
    | (1 << 11) // nG
    ...
    | (0 << 6)  // AP: EL1 RW, EL0 None
    | (0 << 5)  // NS
    | (1 << 2)  // AttrIndx: 1 (Normal NC)
    | (1 << 54) // UXN
    | (1 << 53); // PXN

/// 设备 MMIO（Device-nGnRnE，AttrIndx=2）
pub const PAGE_DEVICE: u64 =
    (1 << 0)   // Valid
    | (1 << 1)  // Page
    | (1 << 10) // AF
    | (1 << 11) // nG
    | (0 << 8)  // SH: Non-shareable
    | (0 << 6)  // AP: EL1 RW, EL0 None
    | (0 << 5)  // NS
    | (2 << 2)  // AttrIndx: 2 (Device-nGnRnE)
    | (1 << 54) // UXN
    | (1 << 53); // PXN
...
pub fn map_page(va: usize, pa: u64, flags: u64) -> Result<(), i32> {
    let ret = unsafe { ffi::map_page(va, pa, flags) };
    if ret == 0 {
        Ok(())
    } else {
        Err(ret)
