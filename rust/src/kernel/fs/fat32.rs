...
impl Fat32Inode {
    ...
    fn encode_short_name(name: &str) -> Result<[u8; 11], &'static str> {
        ...
        let blen = base_upper.len().min(8);
        let elen = ext_upper.len().min(3);
        raw[..blen].copy_from_slice(&base_upper.as_bytes()[..blen]);
        raw[8..8 + elen].copy_from_slice(&ext_upper.as_bytes()[..elen]);
        Ok(raw)
    }
