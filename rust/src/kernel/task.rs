...
impl Drop for PageTable {
    fn drop(&mut self) {
        if self.0 != 0 {
            unsafe { crate::ffi::free_pgd_tree(self.0); }
        }
    }
}
...
pub struct Task {
    ...
    /// 自引用：指向自身 Weak（不贡献 refcount，避免循环引用）。
    pub arc_self: UnsafeCell<Option<Weak<Task>>>,
    pub entry: fn(usize),
    pub arg: usize,
    pub name: &'static str,
...
impl Drop for Task {
    fn drop(&mut self) {
        // 所有物理页由 PageTable::drop → free_page_table_tree → put_page 统一释放
        // code_pa/stack_pa 只是跟踪标记，不在此释放
...
pub fn create_kernel_thread(
    ...
    let task = Task {
        ...
        time_slice,
        prio,
        original_prio: AtomicUsize::new(prio),
        pid: alloc_pid(),
        ppid: 0,
        pgd: PageTable::none(),
        user_sp: 0,
    ...
    unsafe {
        (*arc.arc_self.get()) = Some(Arc::downgrade(&arc));
    }
    return arc;
}
...
pub fn do_fork(parent_regs: *const ffi::PtRegs) -> usize {
    ...
    unsafe {
        ffi::copy_user_pgd(parent_pgd, child_pgd);
    }
    // dump 子进程页表确认拷贝结果
    if crate::config::FORK_DEBUG {
    ...
    let mut task = Task {
        ...
        time_slice,
        prio: parent.prio,
        original_prio: AtomicUsize::new(parent.prio),
        pid: alloc_pid(),
        ppid: parent.pid,
