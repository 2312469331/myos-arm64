...
// ── 全局 virtqueue 状态 ──
static mut VQ_PA: u64 = 0;
static mut VQ_VA: usize = 0;

#[allow(dead_code)]
fn used_offset() -> usize { 8192 }

/// 初始化 virtqueue（legacy 模式使用 QueuePFN）
fn setup_virtqueue(regs: &MmioRegs, qidx: u32) -> Result<(), &'static str> {
    // 选择队列
    ...
    // 分配 4 页连续物理内存（desc + avail + used）
    let queue_pa = unsafe { ffi::alloc_phys_pages(2, 0) };
    if queue_pa == 0 {
        return Err("alloc queue pages failed");
    }
    let queue_va = crate::kernel::pgtbl::phys_to_virt(queue_pa) as *mut u8;

    // 清零
    unsafe { core::ptr::write_bytes(queue_va, 0, 16384); }

    // 写 QueueSize、QueueAlign（GuestPageSize 已在 init 里设好）
    regs.write32(REG_QUEUE_SIZE, VQ_SIZE as u32);
    regs.write32(REG_QUEUE_ALIGN, 4096);

    // 写 QueuePFN = 物理地址 >> 12（页号）
    regs.write32(REG_QUEUE_PFN, (queue_pa >> 12) as u32);

    // 保存全局状态
    unsafe {
        VQ_PA = queue_pa;
        VQ_VA = queue_va as usize;
    }
...
/// 提交 blk 请求并等待完成
fn blk_request(regs: &MmioRegs, is_write: bool, sector: u64, buf: *mut u8, len: u32, wq: Option<&WaitQueue>) -> Result<(), &'static str> {
    let queue_va = unsafe { VQ_VA };
    let queue_pa = unsafe { VQ_PA };
    if queue_va == 0 { return Err("vq not init"); }

    // 分配数据缓冲（头 + 数据 + 状态）
    let layout = alloc::alloc::Layout::from_size_align(16 + len as usize + 1, 16).map_err(|_| "bad layout")?;
    let va = unsafe { alloc::alloc::alloc(layout) };
    if va.is_null() { return Err("alloc buf failed"); }
    let buf_pa = crate::kernel::pgtbl::virt_to_phys(va as usize) as u64;
    ...
    if result != 0 {
        unsafe { alloc::alloc::dealloc(va, layout); }
        return Err("io err");
    }
...
pub fn read_sector(sector: u64, buf: &mut [u8]) -> Result<(), &'static str> {
    let vm = unsafe { ffi::ioremap(0xa003e00, 0x200) };
    if vm.is_null() { return Err("ioremap failed"); }
    let regs = unsafe { MmioRegs::new(vm) };
    blk_request(&regs, false, sector, buf.as_mut_ptr(), buf.len() as u32, None)
}
...
pub fn write_sector(sector: u64, buf: &[u8]) -> Result<(), &'static str> {
    ...
    if vm.is_null() { return Err("ioremap failed"); }
    let regs = unsafe { MmioRegs::new(vm) };
    blk_request(&regs, true, sector, buf.as_ptr() as *mut u8, buf.len() as u32, None)
}
...
impl VirtioBlk {
    pub fn new(mmio_pa: u64, irq_num: u32) -> Result<Arc<Self>, &'static str> {
        ...
        setup_virtqueue(&regs, 0).map_err(|_| "vq init failed")?;
        regs.write32(REG_STATUS, (STATUS_ACK | STATUS_DRIVER | STATUS_FEATURES_OK | STATUS_DRIVER_OK) as u32);
        // 注册 IRQ 处理函数（通过 C 的 irq_register）
        // 注册 IRQ 处理函数
        // 用非空名字，避免 C 侧 uart_puts 出问题
        crate::irq::register_irq(irq_num, virtio_irq_handler, "virtio-blk");
        let blk = Arc::new(VirtioBlk { regs, wq: WaitQueue::new(), irq: irq_num });
...
impl BlockDevice for VirtioBlk {
    fn read_sector(&self, sector: u64, buf: &mut [u8]) -> Result<(), &'static str> {
        blk_request(&self.regs, false, sector, buf.as_mut_ptr(), buf.len() as u32, self.wq_or_poll())
    }
    fn write_sector(&self, sector: u64, buf: &[u8]) -> Result<(), &'static str> {
        blk_request(&self.regs, true, sector, buf.as_ptr() as *mut u8, buf.len() as u32, self.wq_or_poll())
    }
