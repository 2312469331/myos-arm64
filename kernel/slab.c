...
#define ARM64_PAGE_PROT \
	(ARM64_PTE_VALID | ARM64_PTE_TYPE_PAGE | ARM64_PTE_AF | \
	 ARM64_PTE_SH_INNER | ARM64_PTE_AP_RW_KERNEL | \
	 ARM64_PTE_ATTRIDX(0) | ARM64_PTE_UXN | ARM64_PTE_PXN)

#define ARM64_TABLE_PROT \
	(ARM64_PTE_VALID | ARM64_PTE_TYPE_TABLE)
...
static void slab_cache_add_page(struct slab_cache *cache, struct slab_page *sp)
{
	sp->next = cache->partial;
	cache->partial = sp;
}

static void slab_cache_del_page(struct slab_cache *cache, struct slab_page *sp)
{
