...
static unsigned int pages_to_order(size_t pages) {
    ...
    while (p < pages) {
        p <<= 1;
        order++;
        if (order > MAX_VMALLOC_ORDER) return MAX_VMALLOC_ORDER;
    }
    return order;
}
...
static void remove_from_bucket(vmap_area_t *va) {
    size_t pages = (va->va_end - va->va_start) / PAGE_SIZE;
    unsigned int order = pages_to_order(pages);
    if (order > MAX_VMALLOC_ORDER) order = MAX_VMALLOC_ORDER;
...
void vmalloc_init(void) {
    ...
    if (!initial_va) return; // 极端情况：slab 尚未就绪
    ...
    // 挂入红黑树
    spin_lock(&va_tree_lock);
    rb_tree_insert(&va_tree, &initial_va->rb_node);
    spin_unlock(&va_tree_lock);
...
void vfree(void *addr) {
    ...
    vmap_area_t *va = rb_entry(found_node, vmap_area_t, rb_node);
    
    // 释放物理内存
    size_t pages = (va->va_end - va->va_start) / PAGE_SIZE;
    unsigned int phys_order = pages_to_order(pages);
    free_phys_pages(va->phys_start, phys_order);
...
static void print_va_cb(rb_node_t *node, void *data) {
    ...
    printk("[%s] va: 0x%lx - 0x%lx (phys: 0x%lx)\n", 
           status, va->va_start, va->va_end, va->phys_start);
}
...
// ---------------------------------------------------------
// 注意：返回的 vmap_area_t 处于"半死不活"状态：已从空闲桶摘除，还在红黑树中，is_free=0
static vmap_area_t *__get_vm_area(size_t size) {
    ...
    if (required_order > MAX_VMALLOC_ORDER) return NULL;

    vmap_area_t *found_va = NULL;

    // 1. 遍历分桶寻找虚拟空洞 (这部分逻辑与之前完全一样)
    for (unsigned int o = required_order; o <= MAX_VMALLOC_ORDER; o++) {
        if (list_empty(&buckets[o].free_list)) continue;
        spin_lock(&buckets[o].lock);
        if (!list_empty(&buckets[o].free_list)) {
            found_va = list_first_entry(&buckets[o].free_list, vmap_area_t, list_node);
            list_del(&found_va->list_node);
            spin_unlock(&buckets[o].lock);
            break;
        }
        spin_unlock(&buckets[o].lock);
    }
    if (!found_va) return NULL;
...
void *ioremap(phys_addr_t phys_addr, size_t size) {
    if (phys_addr == 0 || size == 0) return NULL;
...
void iounmap(void *addr) {
    ...
    // 归还虚拟地址区间 (注意：绝对不能调 free_phys_pages，因为物理地址不是Buddy给的！)
    __put_vm_area(va);
}